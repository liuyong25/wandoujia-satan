...
(function($)
{
    ...
    p.positionDropdown = function()
    {
        ...
        css[direction === POSITION_ABOVE ? 'bottom' : 'top'] = (height + 16) + 'px';
        container.css(css);
    };
...
(function($)
{
     ...
     */
    p.onEnterKeyPress = function(e, loseFocus)
    {
        var self = this,
            val  = self.val(),
            tag  = self.itemManager().stringToItem(val)
            ;
        if(val.trim() && self.isTagAllowed(tag))
        {
            self.addTags([ tag ]);
    ...
    p.renderTag = function(tag)
    {
        ...
        var parts = self.itemManager().itemToString(tag).split(':');
